...
export function MusicPlayer({ tracks }: MusicPlayerProps) {
  ...
  return (
    ...
    <div className="w-full max-w-sm rounded-2xl p-6 bg-white/30 shadow-2xl backdrop-blur-lg border border-pink-400 font-poppins text-pink-800 flex flex-col items-center">
      <audio ref={audioRef} src={currentTrack.src} onEnded={handleNext} />

      {/* Album Art (larger and on top) */}
      <img src={currentTrack.image} alt={currentTrack.title} className="w-full aspect-square rounded-lg shadow-2xl mb-6" />
      ...
      <div className="flex items-center justify-center space-x-6 mt-8 w-full">
        <button onClick={handlePrev} className="p-2 rounded-full hover:bg-pink-500 transition-colors">
        ...
        <button onClick={handlePlayPause} className="p-4 bg-white/20 rounded-full hover:bg-pink-500 transition-colors shadow-lg">
          {isPlaying ? (
        ...
        <button onClick={handleNext} className="p-2 rounded-full hover:bg-pink-500 transition-colors">
